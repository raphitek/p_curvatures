...
def compute_tree( A, Ad, i, j, end, steps=False):
    r"""
    ...
    if i == end:
        ...
        a1,a2= compute_tree(A,Ad,i+1,2*j,end,steps),compute_tree(A,Ad,i+1,2*j+1,end,steps)

        if steps:
            print('        Computing node('+str(i)+','+str(j)+') of complete binary tree of '+str(end)+' floor')
        a = a1*a2
    A[i][j] = a
    return( a )

def fill_tree( W, i ,j, end, L1, L2, steps=False ):
    r"""
    ...
    """
    if steps:
        print('        Computing node('+str(i)+','+str(j)+') of complete binary tree W of '+str(end)+' floor')

    W[i+1][2*j]   = (W[i][j]%L2[i+1][2*j])
    W[i+1][2*j+1] = (W[i][j]*L1[i+1][2*j])%L2[i+1][2*j+1]
    if i<end-1:
        fill_tree( W, i+1, 2*j, end, L1, L2, steps )
        fill_tree( W, i+1, 2*j+1, end, L1, L2, steps)

def harvey_factorial( N, L, p_list, steps=False):
    r"""
    ...
    if steps:
        print('    Filling bottom floor of trees A and S')
...
def p_curvatures( L, N, steps=False ):
    r""" 
    ...
    if steps:
        print('building companion matrix')

    M = build_matrix( L1, m )

    if steps:
        print('computing prime list')
    ...
    factorial_list = harvey_factorial( N, matrix_list, primes_list, steps )

    if steps:
        print( 'Formating factorial matrices')
